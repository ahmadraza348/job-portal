                ...
                <div class="col-6 col-md-2">
                    <div class="align-end">
                        <select name="sort" id="sort" class="form-control">
                            <option value="0"{{Request::get('sort') == 0 ? 'selected' : ''}}>Latest</option>
                            <option value="1"{{Request::get('sort') == 1 ? 'selected' : ''}}>Oldest</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-4 col-lg-3 sidebar mb-4">
                    <form action=""name="searchForm" id="searchForm">
                        <div class="card border-0 shadow p-4">
                                ...
                                @if ($jobtypes->isNotEmpty())
                                    @foreach ($jobtypes as $item)
                                        <div class="form-check mb-2">
                                            <input {{(in_array($item->id, $jobTypeArray)) ? "checked" : "" }} class="form-check-input " name="jobType" type="checkbox"
                                                value="{{ $item->id }}" id="job-type-{{ $item->id }}">
                                            <label class="form-check-label "
                                                for="job-type-{{ $item->id }}">{{ $item->name }}</label>
                                        </div>
                                    @endforeach
                                @endif
                            </div>
                            ...
                            <a href="{{route('jobs')}}" class="mt-3 btn btn-secondary">Reset</a>
                        </div>
                    </form>
